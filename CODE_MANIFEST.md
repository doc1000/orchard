...
- `builder.py` — `OrchardBuilder.build` (defaults → fused domain+function from shared MiniLM/TF-IDF/JS layers + packaged heads, or cue/JS-only offline fallback; `[]` → MiniLM+TF-IDF semantic or explicit TF-IDF / offline fallback)
...
- `backends/` — TF-IDF + MiniLM + ModernBERT taxonomy features + similarity/linkage + fusion engine + layer registry
...
- `assets/profiles/` — packaged `semantic.json` (0.66/0.34), `domain.json`, `function.json` + `PROVENANCE.md` (`function_raw_js` = AppWorld `functional_raw_js`)
- `fixtures/` — generic reference corpus + sample linkage

### Future (not shipped)

- app-family layer / with-app `0.03` dicts `[proposed]` (Phase 5)
